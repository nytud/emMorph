...
#this script fixes pronouns so that they have the lemma én/te etc.
#as determined by person+number marking e1/e2 etc.
...
if($zarte_ana)
{
	%ppron=qw/e1 én e2 të e3 ő t1 mi t2 ti t3 ők/;
}
else
{
	%ppron=qw/e1 én e2 te e3 ő t1 mi t2 ti t3 ők/;
}
while(<>)
{
	s/abbrperiodPUNCT(.*)S_PUNCT/$1/;
	s/S_(PUNCT|KSZ)/I_$1/g;
	s/S_(?=H?KJ|PER)/H_/g;
	if (/${delim}([^${delim}+]*)\+[^${delim}]+${delim}[^${delim}+]*\+[^${delim}]+${delim}FN\|NM[^${delim}]*\+([et][1-3])/o)
	{
		$pro=$1 eq 'ugyanő'?"ugyan$ppron{$2}":$ppron{$2};
		s/${delim}[^${delim}+]*\+(?=[^${delim}]+${delim}FN\|NM(?:[^${delim}]+\+[et][1-3]|[^${delim}]*\+[et][1-3]\+(?:ACC|NOM)))/"${delim}$pro+"/oe;
	}
	print;
}
