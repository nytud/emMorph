...
#create a perl source file with data structures containing info on suffix tags
BEGIN{
...
#%cat2hum=qw/N FN Adj MN Adv HA Num SZN V IGE Part MN/;
while(<>)
{
	next unless /$tagname:([^;]+)/o;
	$a=$_;
	$tag=$1;
	for(split(/\+/,$tag))
	{
		$sfxtags{$_}++ unless /$stemtags/o;
#		$tag=~s/.*\+//;
		next if $_=~/$stemtags/o;
		next if $mcat{$_};
		$mcat{$_}='I',next unless $a=~/mcat:[^;>]*>(=?)([^;]+)/;
		$dercat{$_}=$1.cat2hum($2);
		$mcat{$_}="D=$dercat{$_}";
	}

}
print '$sfxtags=\'(?:'.join('|',sort keys %sfxtags).")(?=\$|[]+])';\n";
print '%mcat=(\''.join('\',\'',%mcat)."');\n";
print '%dercat=(\''.join('\',\'',%dercat)."');\n";
print '$dertags=\'(?:'.join('|',map(quotemeta,sort keys %dercat)).")(?=\$|[]+])';\n";
print "1;\n";
