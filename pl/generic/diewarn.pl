...
#this provides a warn and a die sub which prefix "WARN:/ERROR:" to the message, respectively
{
my $errorflag;
...
sub dienow
{
	my(@a)=@_;
	$a[0]="ERROR: ".$a[0];
	$a[-1].=" at line ".(caller())[2].", input record: $.\n" if $a[-1]!~/\n$/;
	die @_;
}
...
sub die_if_errors
{
	die "$errorflag fatal error(s) occured.\n" if $errorflag;
}
