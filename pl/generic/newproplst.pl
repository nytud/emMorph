...
#program to list properties in a .propset file not yet listed in the
#proplst.hpl property-encoding definition file
...
#produce property list (part of proplst.hpl) from the propsets file
while(<>)
{
	$l=$_;
	($lr,$prp,$req)=(/^([lr]),(.*?)(?:,(.*?))?;/);
	for(split(/[&,!()|]+/,$prp))
	{
		warn("New property: \"$_\" found in $ARGV:\n$l\n"),
		$Gnprops->{$_}=[$lr,'','',''] if $_ ne '' && !$Gprops->{$_} && !$Gnprops->{$_} && !$Gcmp_prop->{$_};
	}
	$lr=$lr eq 'r'?'l':'r';
        for(split(/[&,!()|]+/,$req))
	{
...
if(keys %$Gnprops)
{
	$a=(dumpsh([$Gnprops],['Gprops']));
#	print $a;
	$"=",\n";
	@a=split(/,?\n/,$a);
	shift(@a);
	pop(@a);
	@a=sort {$a=~/'(.*?)'.*?\['([rl])'/;$aa=$2.$1;$b=~/'(.*?)'.*?\['([rl])'/;$bb=$2.$1;$aa cmp $bb}(@a);
	print "\$Gprops = {\n@a,\n};\n";
	die "New properties found.";
}
