...
while(<>)
{
	...
	{
		s/(\@D\.A\.\+\@)/$1\@D.LCA.+\@/;
                s/(\@P\.[OD]\.\+\@)/$1\@D.LCA.-\@\@C.LCA\@/;
        }
        else
        {
        	s/(\@D\.A\.\+\@)/$1 \@D.LCA.+\@ \@D.LCA.-\@ \@P.LCA.+\@ \@P.LCA.-\@ \@C.LCA\@/;
        }
	if(($lex,$srf,$rest)=/^(.+?[^%]):(.+?)\t(.*)/ and $srf=~/\p{Lu}\p{Ll}|\p{Ll}\p{Lu}/)
	{
#		print ;#"$1\t$2\t$3\n";
		$lsrf="\L$srf";
		print "\@P.LCA.+\@$lex:\@P.LCA.+\@$lsrf\t$rest\n";
		print "\@P.LCA.-\@$lex:\@P.LCA.-\@$srf\t$rest\n";
		next;
	}
	print;
}